...
- umm-border advertise `100.68.1.0/24`
...
```
configure terminal
ip route 100.68.1.0/24 null0
end
write memory
```

Verifikasi: `show bgp ipv4 unicast neighbors 100.64.0.1 advertised-routes` → harus ada `100.68.1.0/24`.
...
| Prefix UMM diiklankan | umm-border | `show bgp ipv4 unicast neighbors 100.64.0.1 advertised-routes` | `100.68.1.0/24` |
| Prefix IDREN diterima | umm-border | `show ip route bgp` | `100.68.0.0/16` B ada |
| Prefix UMM diterima | idren-pop | `show bgp ipv4 unicast` | `100.68.1.0/24` |
